	...
	public abstract class FpsInventoryBase : MonoBehaviour, IInventory, IQuickSlots, INeoSerializableComponent
	{
        ...
        void AddItemsFromPrefabArray(FpsInventoryItemBase[] itemArray)
        {
            ...
            for (int i = 0; i < itemArray.Length; ++i)
            {
                ...
                if (wieldableRootNsgo != null)
                {
                    if (itemArray[i].GetComponent<NeoSerializedGameObject>() == null)
                    {
                        if (SceneSaveInfo.currentActiveScene != null)
                            Debug.LogWarning("Added wieldable to character inventory which does not have a NeoSerializedGameObject component. This means it will not work with the NeoFPS save system. Wieldable: " + itemArray[i].name);

                        item = Instantiate(itemArray[i]);
                    }
                    else
                        item = wieldableRootNsgo.InstantiatePrefab(itemArray[i]);
                }
                else
                    item = Instantiate(itemArray[i]);
