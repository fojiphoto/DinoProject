            ...
            if (m_ParticleCount > 0)
            {
                ...
                for (int i = 0; i < m_ParticleCount; ++i)
                {
                    ...
                    if (m_Particles[i].remainingLifetime < lifeRemain)
                    {
                        ...
                        float scaleValue = (scale.x + scale.y + scale.z) * 0.333333f;

                        var rb = PoolManager.GetPooledObject<Rigidbody>(m_RigidbodyPrefab, pos + rot * m_Particles[i].position, rot * Quaternion.Euler(m_Particles[i].rotation3D), scale);
                        rb.angularVelocity = m_Particles[i].angularVelocity3D;
                        rb.velocity = rot *  m_Particles[i].velocity * scaleValue;
