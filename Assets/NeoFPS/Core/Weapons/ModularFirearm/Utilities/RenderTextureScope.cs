...
namespace NeoFPS.ModularFirearms
{
    ...
    public class RenderTextureScope : MonoBehaviour, INeoSerializableComponent
    {
        ...
        private void Update()
        {
            if (FirstPersonCameraBase.current != null && m_FocalPoint != null)
            {
                // Get vector to camera point
                Vector3 direction = Vector3.Normalize(m_FocalPoint.position - FirstPersonCameraBase.current.transform.position);
